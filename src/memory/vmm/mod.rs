...
pub fn map_to(region: Region, start_address: paddr) -> Result<(),VmmError> {
    let vmm_lock = KERNEL_VMM.lock();
    let mut vmm = vmm.get_mut();
    if !vmm.insert(region) {
        return Err(VmmError::MemUsed);
    }
    if arch_map_to(&vmm.arch_specific, region, start_address) is Err(E) {
        vmm.remove(region.start);
        return Err(E)
    }
    Ok(())
}
...
pub fn map(region: Region) -> Result<(),VmmError> {
    ...
    if arch_map(&vmm.arch_specific, region) is Err(E) {
        vmm.remove(region.start);
        return Err(E)
    }
    Ok(())
...
pub fn unmap(addr: vaddr) -> bool {
    ...
    if let Some(region) = vmm.remove_region(vaddr) {
        arch_unmap(&vmm.arch_specific, region);
        true
    } else {
        false
    }
...
impl VMM {
   ...
   pub fn remove_region(&mut self, address: usize) -> Option<(Region,Option<Region>)>
       self.drain_filter(|region| region.contains(address))
           // should contain /at most/ one region
           .next()
   }
}
